...
struct ToolsView: View {
            ...
            Group {
                if app.selected == nil {
                    EmptyStateView(symbol: "wrench.and.screwdriver",
                                   title: "Geen server geselecteerd",
                                   message: "Kies eerst een server in het tabblad Server.")
                } else {
                    list
                }
            }
    ...
    private var list: some View {
        List {
            Section {
                ...
                Text("Alle tools draaien op de geselecteerde server.")
            }
            ...
            Section("Hardware") {
                ...
                ToolRow(title: "Sensoren", symbol: "sensor.fill", tint: Theme.C.teal) { SensorsDetailView() }
                ToolRow(title: "Opslag & SMART", symbol: "internaldrive.fill",
                        tint: Theme.C.magenta) { SmartDetailView() }
                ToolRow(title: "Netwerkinterfaces", symbol: "cable.connector",
                        tint: Theme.C.green) { NetworkDetailView() }
                ToolRow(title: "Hardware-overzicht", symbol: "server.rack",
            ...
            Section("Systeem") {
                ...
                ToolRow(title: "Processen", symbol: "list.bullet.rectangle",
    ...
    private var speedtestSubtitle: String? {
        ...
        return sys.hasSpeedtest ? nil : "vereist speedtest op de server"
    }
}
    ...
    var body: some View {
        DetailScroll {
            if let s = app.latest {
                Card {
                    VStack(alignment: .leading, spacing: 12) {
                        ...
                        HStack(spacing: 16) {
                            legend("Totaal", Theme.C.blue, s.cpu.total)
                ...
                Card {
                    VStack(alignment: .leading, spacing: 10) {
                        HStack {
                            ...
                            Spacer()
                            Toggle("Historie", isOn: $showHistory)
                                .labelsHidden()
                        }
                ...
                InfoCard(title: "Belasting", symbol: "gauge.with.dots.needle.50percent", tint: Theme.C.green) {
                    InfoRow(label: "Load average", value: s.cpu.loadLine)
                    InfoRow(label: "Processen", value: "\(s.cpu.procsRunning) actief / \(s.cpu.procsTotal) totaal")
            ...
            if let sys = app.system {
                InfoCard(title: "Specificaties", symbol: "cpu.fill", tint: Theme.C.magenta) {
                    InfoRow(label: "Model", value: sys.cpu.model)
                    InfoRow(label: "Cores / threads", value: "\(sys.cpu.coresPhysical) / \(sys.cpu.threads)")
                    if let m = sys.cpu.maxMhz, m > 0 { InfoRow(label: "Max frequentie", value: "\(m) MHz") }
                    if let g = sys.cpu.governor { InfoRow(label: "Governor", value: g) }
                }
            }
        }
        .navigationTitle("CPU")
...
struct ProcessesView: View {
    ...
    var body: some View {
        AsyncLoad(path: "v1/tools/processes", refreshInterval: 5) { (r: ProcessesResult) in
            ...
            .searchable(text: $query, prompt: "Zoek proces")
        }
        .navigationTitle("Processen")
    }
        ...
        AsyncLoad(path: "v1/tools/updates", refreshInterval: 120) { (r: UpdatesInfo) in
            DetailScroll {
                    ...
                    Card {
                        HStack(spacing: 10) {
                            ...
                            VStack(alignment: .leading, spacing: 2) {
                                Text("Herstart vereist").font(.headline).foregroundStyle(Theme.C.text)
                ...
                HStack(spacing: Theme.M.cardGap) {
                    statCard("\(r.upgradable)", "Beschikbaar", Theme.C.blue)
                    statCard("\(r.security)", "Security", r.security > 0 ? Theme.C.crit : Theme.C.ok)
                }

                InfoCard(title: "Status", symbol: "gear.badge", tint: Theme.C.orange) {
                    InfoRow(label: "Unattended upgrades",
                            value: r.unattendedUpgrades ? "aan" : "uit",
                            tint: r.unattendedUpgrades ? Theme.C.ok : Theme.C.warn)
                    if let t = r.lastAptUpdate, t > 0 {
                        InfoRow(label: "Cache bijgewerkt", value: Fmt.ago(Double(t)))
                    }
                }

                if !r.packages.isEmpty {
                    Text("Pakketten").font(.headline).foregroundStyle(Theme.C.text).padding(.top, 6)
                ...
                Text("Alleen-lezen: de app installeert niets. Zie de analyse, hoofdstuk 7.")
                    .font(.caption2)
                    .foregroundStyle(Theme.C.textTertiary)
                    .padding(.top, 8)
            }
        ...
        AsyncLoad(path: "v1/tools/uptime", refreshInterval: 30) { (r: UptimeInfo) in
            DetailScroll {
                ...
                InfoCard(title: "Uptime", symbol: "clock.fill", tint: Theme.C.gray) {
                    ...
                    InfoRow(label: "Bezetting sinds boot", value: Fmt.percent(r.busyRatio * 100))
                }

                if !r.boots.isEmpty {
                    Card {
                        VStack(alignment: .leading, spacing: 8) {
                            Text("Laatste boots").font(.headline).foregroundStyle(Theme.C.text)
