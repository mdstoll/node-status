...
struct MetricsView: View {
            ...
            Group {
                if app.selected == nil {
                    EmptyStateView(
                        ...
                        message: "Koppel eerst een server in het tabblad Server."
                    )
                } else {
                    content
                }
            ...
            Card {
                VStack(alignment: .leading, spacing: 18) {
                    ...
                    Grid(horizontalSpacing: 16, verticalSpacing: 14) {
                        GridRow {
                            KeyValue(key: "Model", value: sys.modelLine)
                        ...
                        GridRow {
                            ...
                            KeyValue(key: "Reboot Date", value: Fmt.date(sys.bootTime))
                        }
                    }
                }
            }
    ...
    /// Uptime telt door op de wandklok, dus zonder extra serververkeer.
    private func liveUptime(_ sys: SystemInfo) -> Double {
        guard sys.bootTime > 0 else { return sys.uptimeS }
        return tick.timeIntervalSince1970 - Double(sys.bootTime)
    }
    ...
    private func badge(count: Int, symbol: String, color: Color, label: String) -> some View {
        HStack(spacing: 4) {
            ...
            Text("\(label) \(count)").foregroundStyle(Theme.C.textSecondary)
        }
        .font(.caption)
    }
