...
struct PairingInfo: Sendable, Equatable, Identifiable {
    ...
    /// Leest `serverinfo://enroll?h=…&p=…&fp=…&c=…&n=…` uit de QR.
    init?(url: URL) {
        guard url.scheme == "serverinfo", url.host == "enroll",
...
enum EnrollmentError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .noWindow:
            "Er staat geen koppelvenster open. Draai op de server: sudo serverinfo-agent enroll --new"
        }
    }
}
